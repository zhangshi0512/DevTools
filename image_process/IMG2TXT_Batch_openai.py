...
def create_batch_jsonl(image_files, api_model, max_tokens):
    ...
    请求中包含 max_tokens 参数，调用 /v1/chat/completions 接口，内容要求将图中表格转换为 Markdown 格式。
    """
    lines = []
...
class OpenAIBatchConverterGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("OpenAI Batch API 图像转 Markdown")
    ...
    def create_widgets(self):
        # API 密钥输入
        tk.Label(self, text="请输入 OpenAI API 密钥：").pack(pady=(10, 0))
        self.api_key_entry = tk.Entry(self, width=50)
        self.api_key_entry.pack(pady=(0, 10))

        # 模型名称输入（默认 "gpt-3.5-turbo-0125" 这里示例用的模型可根据需要替换为图像处理模型，如 gpt-4-vision-preview）
        tk.Label(self, text="请输入模型名称（例如：gpt-3.5-turbo-0125）：").pack(pady=(10, 0))
        self.model_entry = tk.Entry(self, width=30)
        self.model_entry.insert(0, "gpt-3.5-turbo-0125")
    ...
    def start_batch_conversion(self):
        ...
        api_model = self.model_entry.get().strip() or "gpt-3.5-turbo-0125"
        try:
